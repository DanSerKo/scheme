...
// Tools
std::pair<std::shared_ptr<Object>, std::shared_ptr<Object>> Seter(std::shared_ptr<Object> node,
                                                                  Interpreter*) {
    if (!Is<Cell>(node)) {
    ...
    if (!Is<Symbol>(first)) {
        throw NameError("Invalid define name");
    }
    if (!Is<Cell>(second)) {
        throw RuntimeError("pair isn't correct for define");
    }
...
std::shared_ptr<Object> IsCell(std::shared_ptr<Object> node, Interpreter* interpreter) {
    ...
    if (Is<Symbol>(first) && As<Symbol>(first)->GetName() != "#f" &&
        ...
        if (second == nullptr) {
            return std::make_shared<Cell>(ToDo(first, interpreter), nullptr);
        }
...
std::shared_ptr<Object> ToDo(std::shared_ptr<Object> node, Interpreter* interpreter) {
    ...
    if (interpreter->defined_.count(name)) {
        return ToDo(interpreter->defined_[name], interpreter);
    }
    throw NameError("what?");
}
...
std::shared_ptr<Object> Define(std::shared_ptr<Object> node, Interpreter* interpreter) {
    auto [first, second] = Seter(node, interpreter);
    second = As<Cell>(second)->GetFirst();
    second = ToDo(second, interpreter);
    interpreter->defined_[As<Symbol>(first)->GetName()] = second;
    return MakeQuote(nullptr, interpreter);
}

std::shared_ptr<Object> Set(std::shared_ptr<Object> node, Interpreter* interpreter) {
    auto [first, second] = Seter(node, interpreter);
    if (!interpreter->defined_.count(As<Symbol>(first)->GetName())) {
        throw NameError("this arg didn't define");
    }
    second = As<Cell>(second)->GetFirst();
    second = ToDo(second, interpreter);
    interpreter->defined_[As<Symbol>(first)->GetName()] = second;
...
std::shared_ptr<Object> SetCar(std::shared_ptr<Object> node, Interpreter* interpreter) {
    auto [first, second] = Seter(node, interpreter);
    if (!interpreter->defined_.count(As<Symbol>(first)->GetName())) {
        throw NameError("this arg didn't define");
    }
    first = interpreter->defined_[As<Symbol>(first)->GetName()];
    if (!Is<Cell>(first)) {
        throw RuntimeError("arg isn't pair");
    }
    second = As<Cell>(second)->GetFirst();
    second = ToDo(second, interpreter);

    first = Demining(Cdr(first, interpreter), interpreter);

    first = MakeQuote(std::make_shared<Cell>(second, first), interpreter);
    interpreter->defined_[As<Symbol>(first)->GetName()] = first;

    return nullptr;
}

std::shared_ptr<Object> SetCdr(std::shared_ptr<Object> node, Interpreter* interpreter) {
    auto [first, second] = Seter(node, interpreter);
    if (!interpreter->defined_.count(As<Symbol>(first)->GetName())) {
        throw NameError("this arg didn't define");
    }
    ...
    second = ToDo(second, interpreter);
    first = Demining(Car(first, interpreter), interpreter);
    first = MakeQuote(std::make_shared<Cell>(first, second), interpreter);
    interpreter->defined_[As<Symbol>(first)->GetName()] = first;
